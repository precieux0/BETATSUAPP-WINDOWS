...
class MangaSourcesRepository @Inject constructor(
	...
	// Local custom sources bundled with this app (name format: CUSTOM:name|Title|locale|TYPE)
	private val customLocalSources: Set<org.koitharu.kotatsu.core.model.MangaSource> = setOf(
		// anime-sama.store
		org.koitharu.kotatsu.core.model.CustomMangaSource(
			name = "CUSTOM:anime-sama.store",
			title = "anime-sama.store",
			locale = "en",
			contentType = org.koitharu.kotatsu.parsers.model.ContentType.MANGA,
		),
		// MangaDex (recommendation)
		org.koitharu.kotatsu.core.model.CustomMangaSource(
			name = "CUSTOM:mangadex.org",
			title = "MangaDex",
			locale = "en",
			contentType = org.koitharu.kotatsu.parsers.model.ContentType.MANGA,
		),
		org.koitharu.kotatsu.core.model.CustomMangaSource(
			name = "CUSTOM:mangapark.net",
			title = "MangaPark",
			locale = "en",
			contentType = org.koitharu.kotatsu.parsers.model.ContentType.MANGA,
		),
		org.koitharu.kotatsu.core.model.CustomMangaSource(
			name = "CUSTOM:mangahere.cc",
			title = "MangaHere",
			locale = "en",
			contentType = org.koitharu.kotatsu.parsers.model.ContentType.MANGA,
		),
	...
	// Combined view of parser-backed sources and our local custom sources
	private val allSourcesCombined: Set<org.koitharu.kotatsu.core.model.MangaSource> by lazy {
		val set = java.util.LinkedHashSet<org.koitharu.kotatsu.core.model.MangaSource>()
		set.addAll(allMangaSources)
		set.addAll(customLocalSources)
	...
	suspend fun getDisabledSources(): Set<MangaSource> {
		...
		val result = EnumSet.copyOf(allSourcesCombined)
	...
	fun observeAll(): Flow<List<Pair<MangaSource, Boolean>>> = dao.observeAll().map { entities ->
		...
		for (entity in entities) {
			...
			if (source in allSourcesCombined) {
	...
	fun observeHasNewSources(): Flow<Boolean> = observeIsNsfwDisabled().map { skipNsfw ->
		...
		sources.isNotEmpty() && sources.size != allSourcesCombined.size
	}.onStart { assimilateNewSources() }
	...
	private fun String.toMangaSourceOrNull(): MangaSource? {
		if (this.startsWith("CUSTOM:")) {
			val payload = this.substringAfter(':')
			...
			return org.koitharu.kotatsu.core.model.CustomMangaSource(name = this, title = title, locale = locale, contentType = type)
		}
